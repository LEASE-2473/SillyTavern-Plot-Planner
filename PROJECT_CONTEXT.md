...
- 当前版本：`2.1.0`。
- 已实现弱化配置区的剧情生成面板、可配置聊天标签过滤、调试提示词查看面板、插件内表单控件独立深色主题与任务文本框可读颜色、固定格式剧情草案生成、上下文收集预览、草案商讨记忆、按聊天持久化、独立子任务拆解提示词、失败后手动重新拆解剧情、任务执行控制、清空剧情、错误/重试/取消/超时处理、不改写正文且不额外消耗判定模型的完成标签推进。
