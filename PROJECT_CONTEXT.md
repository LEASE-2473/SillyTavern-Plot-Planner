...
- 剧情生成界面：默认展示上下文选择、剧情方向/结局、期望任务节点数量和生成草案按钮；API、Profile 与提示词配置折叠在高级设置中。
...
- 当前版本：`2.0.6`。
- 已实现默认可见的剧情生成面板、插件内表单控件独立深色主题与任务文本框可读颜色、固定格式剧情草案生成、上下文收集预览、草案商讨记忆、按聊天持久化、独立子任务拆解提示词、任务拆解 JSON 修复请求、任务执行控制、清空剧情、错误/重试/取消/超时处理、不改写正文且不额外消耗判定模型的完成标签推进。
- 本目录是当前唯一项目根目录。
