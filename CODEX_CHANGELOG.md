...
统一修复各功能区之间的状态串扰：剧情规划页发起请求不应因 API 设置页被隐藏而读取错误模型；生成草案不应覆盖 API 设置；剧情草案与任务链需要独立储存，并允许通过 API 重新生成任务链。

### 主要修改内容

- 移除基于 jQuery `:visible` 选择模型的逻辑，改由模型文本输入框作为请求与 Profile 保存的唯一数据源；模型下拉框变更时同步该值。
- 移除生成草案时自动保存 API Profile 的行为，API 配置仅由用户明确保存、新建或删除 Profile 时持久化。
- 将聊天状态拆分为 `plot_planner_planning_state` 与 `plot_planner_task_state`，并为旧版 `plot_planner_state` 增加自动迁移。
- 在任务链区增加“API 重新生成任务链”；新响应完成解析与内容校验后才原子替换旧任务，失败时保留旧任务和执行进度。
- 将清理操作拆分为“清空草案”“清空任务链”“全部清空”，并移动聊天标签过滤设置到剧情规划功能区。
- 更新移动端操作布局，版本提升到 `2.1.5`。
...
- manifest JSON 与版本断言：通过，输出 `manifest 2.1.5 ok`。
- 状态/API/任务重生成静态回归断言：通过，确认独立状态键、草案生成不再保存 Profile、模型使用单一数据源、任务链在校验后才替换，并且没有重复静态 DOM ID。
- `git -C .\plot-planner diff --check`：通过，仅有工作区既有的 LF/CRLF 转换提示。

### 未完成事项

- 尚未在真实 SillyTavern 桌面端和手机端执行端到端请求与状态迁移测试。

### 已知风险与后续建议

- 任务链重生成仍受所选模型的 JSON 输出质量及上游最大输出限制影响；失败时会保留旧任务链，可再次重试。
- 旧状态迁移依赖 SillyTavern 正常保存聊天元数据，建议首次升级后切换聊天并刷新确认草案和任务链均可恢复。
...
修复剧情大纲可要求 25 个阶段、任务拆解却被硬限制为 10 个节点的问题，并降低 Gemini 因输出额度耗尽而返回空正文的频率。

### 主要修改内容

- 将任务节点范围从 1–10 调整为 5–50，生成大纲与任务拆解统一使用同一数量规范化函数。
- 删除拆解阶段写死的 `Math.min(10, ...)` 限制。
- 拆解输出额度按节点数动态分配：最低 16,384、每节点 2,048、最高 65,535 tokens；25 节点对应 51,200 tokens。
- 独立 API 请求显式发送 `max_tokens`，与 SillyTavern 当前模型模式使用同一拆解额度。
- 独立 API 返回 `finish_reason: length` 且正文为空时显示明确的长度耗尽错误。
- 插件版本由 `2.1.2` 更新为 `2.1.3`。
...
- `node --check .\plot-planner\index.js`：通过，无语法错误输出。
- manifest 版本检查：通过，确认版本为 `2.1.3`。
- 节点与额度单元断言：通过，确认 1 会收敛为 5、25 保持为 25、999 收敛为 50；5/10/25/50 节点分别得到 16,384/20,480/51,200/65,535 tokens。
- 静态断言：通过，确认旧 `max="10"`、`Math.min(10, ...)`、`responseLength: 4096` 均已移除，独立 API 会发送 `max_tokens`。
- `git -C .\plot-planner diff --check`：通过，仅输出现有行尾转换提示。
- VPS 只读复查：SillyTavern、Vertex Proxy、Caddy 均正常；12:50 后无新增代理错误。日志中 `4096 + finish_reason: length` 多次出现，而 `65535 + finish_reason: stop` 有成功记录。

### 未完成事项

- 尚未在真实 SillyTavern UI 中端到端生成并拆解 25 个任务节点。

### 已知风险与后续建议

- 实际最大输出仍受模型和上游 API 限制；当前代理日志显示支持请求 65,535，但其他模型可能返回参数错误或采用更低上限。
- 大纲本身约 21k tokens 时，25 节点拆解请求总体 token 消耗较高。
